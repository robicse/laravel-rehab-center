    ...
    public function edit($id)
    {
        try {
            ...
            if ($User->user_type == 'Admin') {
                $data = RehabCenter::with('rehabcenter')->whereuser_id($User->id)->findOrFail(decrypt($id));
            } else {
                $data = RehabCenter::with('rehabcenter')->findOrFail(decrypt($id));
            }
            return view('backend.writer.rehab_centers.edit')->with('rehabdata', $data);
        } catch (\Exception $e) {
            DB::rollBack();
            $response = ErrorTryCatch::createResponse(false, 500, 'Internal Server Error.', null);
            Toastr::error($response['message'], "Error");
