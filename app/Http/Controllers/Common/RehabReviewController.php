...
class RehabReviewController extends Controller
{
    ...
    {
       try {
            $data = RehabReview::latest();
            if ($request->ajax()) {
                return Datatables::of($data)
                    ->addIndexColumn()
                    ->addColumn('action', function ($data) {
                        $btn = '<a href=' . route(request()->segment(1) . '.rehab-review-lists.edit', (encrypt($data->id))) . ' class="btn btn-info btn-sm waves-effect" style="margin-left: 5px"><i class="fa fa-edit"></i></a><button type="button"   title="Delete"   id="deleteBtn" rid="' . $data->id . '" class="btn btn-danger btn-sm waves-effect"><i class="fas fa-trash"></i></button>';
                        return $btn;
                    })
            ...
            return view('backend.common.rehab_reviews.index');
        } catch (\Exception $e) {
